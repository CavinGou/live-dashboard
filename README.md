...
- 实时状态通过 SSE 推送，断线时自动回退到轮询
- 背景随当前活动切换；自定活动图片优先，未配置时使用 Infinity 壁纸并按时间轮换，接口异常时回退到 Bing 每日壁纸
- 自定义应用映射：挂载一个 JSON 文件即可新增/覆盖应用名与文案，无需改代码重构建
- 系统托盘常驻 + AFK 检测（看视频/听歌时自动豁免）
- 音乐检测（Spotify、QQ音乐、网易云等，前端支持专辑封面展示）
...
- `backgrounds` 段按 `appId` / `appName` / `title` 匹配当前活动并设置背景图 URL；未匹配时自动使用 Infinity 壁纸（默认每 30 分钟轮换，可用 `WALLPAPER_ROTATION_MINUTES` 调整），接口异常时回退到 Bing 每日壁纸
